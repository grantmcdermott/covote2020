...
Consider this a "sister" repo to my [**covote** repo](https://github.com/grantmcdermott/covote) (the latter being focused on the 2016 election). The analysis here is based on preliminary and incomplete data, but should become increasingly accurate as vote tallies are finalised.

The below scripts were most recently run at **`r Sys.time()`**.
...
We start with some other (i.e. non-2020 election) data, which we'll cache to save having to re-download later.

```{r other_data, message=FALSE, cache=TRUE}
# 2016 election data ------------------------------------------------------
...
Next, the 2020 election data. **NOTE: Again, these data should be considered incomplete and preliminary!**
